...
class ThumbnailJob(QRunnable):
    ...
    def __init__(
        ...
        *,
        ...
        self._cache_rel = cache_rel if cache_rel is not None else rel
    ...
    def run(self) -> None:  # pragma: no cover - executed in worker thread
        ...
        if self._known_stamp is not None:
            if self._known_stamp == actual_stamp:
                ...
                old_path = generate_cache_path(self._album_root, self._cache_rel, self._size, self._known_stamp)
                safe_unlink(old_path)

        # 3. Calculate Cache Path
        cache_path = generate_cache_path(self._album_root, self._cache_rel, self._size, actual_stamp)
